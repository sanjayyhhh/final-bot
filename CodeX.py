...
SERVER_COUNT_CHANNEL_ID = 1535959104407994409  # Replace with your server count channel ID
USER_COUNT_CHANNEL_ID = 1535959104407994409    # Replace with your user count channel ID
LOG_CHANNEL_ID = 1535959104407994409 # Replace with the channel ID for join/leave logs


client = zyrox()
tree = client.tree

# --- স্টাইল ইঞ্জিন ইনিশিয়াজেশন ও গার্ড ---
client.style_started = False
...
# --- স্টাইল ইঞ্জিনের জন্য ব্যাকগ্রাউন্ড টাস্ক ফাংশন ---
async def run_presets_task(bot):
    try:
        print("[DisplayNameStyles] Dispatched background style service...")
        # অটোমেটিক প্রিসেট রোটেশন মোডে স্টার্ট করা
        report = await ProfileStyleService.initialize(bot, {
            "styleMode": "rotate", 
            "runCompatibilityTests": False
        })
        print(f"[DisplayNameStyles] Task complete. Supported: {report.get('endpointSupported')}")
    except Exception as e:
...
╚██████╗╚██████╔╝██████╔╝███████╗██╔╝ ██╗
 ╚═════╝ ╚═════╝ ╚═════╝ ╚══════╝╚═╝  ╚═╝
    ...
    # --- অন-রেডি ব্লকে স্টাইল ইঞ্জিন ব্যাকগ্রাউন্ড টাস্ক চালু করা ---
    if not getattr(client, "style_started", False):
        client.style_started = True
        client.loop.create_task(run_presets_task(client))
...
async def on_guild_join(guild: discord.Guild):
    ...
    if log_channel:
        await log_channel.send(f"NEON~V2 has been added to the server: **{guild.name}** (ID: `{guild.id}`)")

# NOTE: An on_command_completion listener used to live here. It sent every
# command anyone ran, anywhere, to a Discord webhook owned by the original
# developer (user/server/channel included), with no notice to admins.
# Removed entirely — see README.md item 1.
