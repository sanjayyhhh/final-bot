...
class MusicControlView(View):
    ...
    @discord.ui.button(emoji="<a:DJ_MUSIC:1535979683949060208>", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(emoji="<:rewind:1500793294865563658>", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(emoji="<:zpause:1448949283423522928>", style=discord.ButtonStyle.success)
    async def pause_button(self, interaction: discord.Interaction, button: Button):
        if self.player.paused:
            await self.player.pause(False)

            await self.player.channel.edit(status=f"<:zpause:1448949283423522928> Playing: {self.player.current.title}")
            button.emoji = "<:zpause:1448949283423522928>" 
            await interaction.response.edit_message(view=self)

        elif self.player.playing:
            await self.player.pause(True)
            await self.player.channel.edit(status=f"<:zpause:1448949283423522928>  Paused: {self.player.current.title}")
            button.emoji = "<:next:1448949316109733920>"
            await interaction.response.edit_message(view=self)


    @discord.ui.button(emoji="<:next:1448949316109733920>", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(emoji="<a:loading:1538161262997930094>", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(emoji="<:zpause:1448949283423522928>", style=discord.ButtonStyle.secondary)
...
class Music(commands.Cog):
    ...
    async def connect_nodes(self) -> None:
        nodes = [wavelink.Node(uri="http://sg1-nodelink.nyxbot.app:3000", password="nyxbot.app/support")]
    ...
    async def display_player_embed(self, player, track, ctx, autoplay=False):
        if track.artwork:
            ...
            embed.add_field(name="Source", value=f"[<a:YOUTUBE:1538164795084382218>  Listen on Spotify]({track.uri})" if "spotify" in track.source else f"[<:jiosaavn:1538162654097580113> Listen on JioSaavn]({track.uri})" if "jiosaavn" in track.source else f"[<a:DJ_MUSIC:1535979683949060208> Listen on SoundCloud]({track.uri})" if "soundcloud" in track.source else f"[<a:YOUTUBE:1538164795084382218> Listen on YouTube]({track.uri})")
    ...
    async def play_source(self, ctx, query):
        if not ctx.author.voice:
            await ctx.send(embed=discord.Embed(description="<a:Warningggg:1536000415601004564> you need to be in a voice channel to use this command.", color=0xFF0000))
            return
    ...
    async def autoplay(self, ctx: commands.Context):
        vc = ctx.voice_client
        if not vc or not vc.playing:
            await ctx.send(embed=discord.Embed(description="<a:Warningggg:1536000415601004564> No song is currently playing.", color=0xFF0000))
            return

        if not ctx.author.voice or ctx.author.voice.channel.id != vc.channel.id:
            await ctx.send(embed=discord.Embed(description="<a:Warningggg:1536000415601004564> You need to be in the same voice channel as me to use this command.", color=0xFF0000))
            return
        ...
        if vc:
            ...
            await ctx.send(embed=discord.Embed(description=f"<a:Verified:1535986177264324628> Autoplay {'enabled' if vc.autoplay == wavelink.AutoPlayMode.enabled else 'disabled'} by {ctx.author.mention}.", color=0xFF0000))
    ...
    async def loop(self, ctx: commands.Context):
        ...
        if vc:
            ...
            await ctx.send(embed=discord.Embed(description=f"<a:Verified:1535986177264324628> Loop {'enabled' if vc.queue.mode == wavelink.QueueMode.loop else 'disabled'} by {ctx.author.mention}.", color=0xFF0000))
        else:
    ...
    async def pause(self, ctx: commands.Context):
        vc = ctx.voice_client
        if not vc or not vc.playing:
            await ctx.send(embed=discord.Embed(description="<a:Warningggg:1536000415601004564> No song is currently playing.", color=0x000000))
            return
        ...
        if vc and vc.playing and not vc.paused:
            await vc.pause(True)
            await vc.channel.edit(status=f"<a:DJ_MUSIC:1535979683949060208> Paused: {vc.current.title}")
            await ctx.send(embed=discord.Embed(description=f"Paused by {ctx.author.mention}.", color=0x000000))
        else:
            await ctx.send(embed=discord.Embed(description="<a:Warningggg:1536000415601004564>   Nothing is playing or already paused.", color=0xFF0000))
    ...
    async def resume(self, ctx: commands.Context):
        ...
        if vc and vc.paused:
            ...
            await vc.channel.edit(status=f"<a:DJ_MUSIC:1535979683949060208> Playing: {vc.current.title}")
    ...
    async def skip(self, ctx: commands.Context):
        ...
        if vc and vc.playing and not vc.queue.is_empty:
            ...
            await ctx.send(embed=discord.Embed(description="<a:Warningggg:1536000415601004564> No song is playing or in the queue to skip.", color=0xFF0000))
    ...
    async def volume(self, ctx: commands.Context, level: int):
        ...
        if not vc:
            await ctx.send(embed=discord.Embed(description="<a:Warningggg:1536000415601004564> I'm not connected to a voice channel.", color=0xFF0000))
            return
        ...
        if vc:
            if 1 <= level <= 150:
                ...
                await ctx.send(embed=discord.Embed(description=f"<a:DJ_MUSIC:1535979683949060208> Volume set to {level}% by {ctx.author.mention}.", color=0xFF0000))
            else:
                await ctx.send(embed=discord.Embed(description="<a:Warningggg:1536000415601004564> Volume must be between 1 and 150.", color=0xFF0000))
        else:
    ...
    async def queue(self, ctx: commands.Context):
        ...
        if not vc or not vc.queue or vc.queue.is_empty:
            await ctx.send(embed=discord.Embed(description="<a:Warningggg:1536000415601004564> The queue is currently empty.", color=0xFF0000))
            return
    ...
    async def clearqueue(self, ctx: commands.Context):
        ...
        if not vc or not vc.queue or vc.queue.is_empty:
            await ctx.send(embed=discord.Embed(description="<a:Warningggg:1536000415601004564> No Queue to clear.", color=0xFF0000))
            return
    ...
    async def replay(self, ctx: commands.Context):
        ...
        if not vc or not vc.playing:
            await ctx.send(embed=discord.Embed(description="<a:Warningggg:1536000415601004564> I'm not connected to any voice channel.", color=0xFF0000))
            return
    ...
    async def on_wavelink_track_start(self, payload: wavelink.TrackStartEventPayload):
        ...
        if voice_channel:
            await voice_channel.edit(status=f"<a:DJ_MUSIC:1535979683949060208> Playing: {track.title}")  # type: ignore
