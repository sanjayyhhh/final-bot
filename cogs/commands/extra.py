...
tick = "<a:Verified:1535986177264324628>>"
cross = "<a:Cross:1536000244716802072>"
...
class RoleInfoView(View):
  ...
  @discord.ui.button(label='Show Permissions',  emoji="<a:pepe_coding:1538159144719360140> ", style=discord.ButtonStyle.secondary)
  ...
  async def show_overwrites(self, interaction: discord.Interaction, button: Button):
      ...
      for target, perms in self.channel.overwrites.items():
          ...
          overwrites.append(f"**For {target.name}**\n" +
                            "\n".join(f"  * **{perm}:** {'<a:Verified:1535986177264324628>>' if value else '<a:Cross:1536000244716802072>' if value is False else '⛔'}" for perm, value in permissions.items()))
      ...
      embed.set_footer(text="<a:Verified:1535986177264324628>> = Allowed, <a:Cross:1536000244716802072> = Denied, ⛔ = None")
      await interaction.response.send_message(embed=embed, ephemeral=True)
...
class Extra(commands.Cog):
  ...
  async def uptime(self, ctx):
      ...
      embed = discord.Embed(title=f"NEON~V2 Manager Uptime", color=self.color)
      embed.add_field(name="__UTC__", value=f"<a:Warningggg:1536000415601004564> {uptime_string}\n\n", inline=False)
      embed.add_field(name="__Online Duration__", value=f"<a:timer:1536982553636905031> {uptime_duration_string}", inline=False)
  ...
  async def serverinfo(self, ctx):
        ...
        embed.add_field(
            name="**__About__**",
            value=f"**Name : ** {ctx.guild.name}\n**ID :** {ctx.guild.id}\n**Owner <a:ublue_crown_swift:1538162788730675230> :** {ctx.guild.owner} (<@{ctx.guild.owner_id}>)\n**Created At : ** {c_at}\n**Members :** {len(ctx.guild.members)}",
            inline=False
        )
        ...
        if ctx.guild.features:
            features = "\n".join([f"<a:Verified:1535986177264324628>>: {feature[:1].upper() + feature[1:].lower().replace('_', ' ')}" for feature in ctx.guild.features])
            embed.add_field(
                name="**__Features__**",
        ...
        embed.add_field(
            ...
            value=f"Level: {ctx.guild.premium_tier} [<a:Nitro_Boost:1535986050189623380>{ctx.guild.premium_subscription_count} boosts]",
            inline=False
        )
...
**ID:** {member.id}
**Nickname:** {nickk}
**Bot?:** {'<a:Verified:1535986177264324628>> Yes' if member.bot else '<a:Cross:1536000244716802072> No'}
**Badges:** {badges}
...
**Color:** {member.color if member.color else '99aab5'}
  ...
  async def boosts(self, ctx):
    await ctx.send(
      embed=discord.Embed(title=f"<a:Nitro_Boost:1535986050189623380> Boosts Count Of {ctx.guild.name}",
  ...
  async def report(self, ctx, *, bug):
    ...
    confirm_embed = discord.Embed(title="<a:Verified:1535986177264324628> Bug Reported",
