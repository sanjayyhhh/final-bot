...
class TimeSelect(Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(description=f"**Added Global No Prefix**:\n**🧑‍🤝‍🧑 User**: **[{self.user}](https://discord.com/users/{self.user.id})**\n**<a:announce:1536982399152558170> User Mention**: {self.user.mention}\n**<a:pw_pin:1535986399159914600> User ID**: {self.user.id}\n\n__**Additional Info**__:\n**<a:BAN:1536984960928321537> Added By**: **[{self.author.display_name}](https://discord.com/users/{self.author.id})**\n**⏰ Expiry Time:** {expiry_text}\n**<a:Nitro_Boost:1535986050189623380> Timestamp:** {expiry_timestamp}", color=0xFF0000)
...
class NoPrefix(commands.Cog):
            ...
            if expired_users:
                ...
                for user_id in expired_users:
                    ...
                    if user:
                        ...
                        embed = discord.Embed(
                            description=f"<a:Warningggg:1536000415601004564> Your No Prefix status has **Expired**. You will now require the prefix to use commands.",
                            color=0xFF0000
                        )
                        embed.set_author(name="No Prefix Expired", icon_url=user.avatar.url if user.avatar else user.default_avatar.url)
                        
                        embed.set_footer(text="NEON~V2  - No Prefix, Join support to regain access.")
    ...
    async def add_np(self, user, duration):
        ...
        embed = discord.Embed(
                            ...
                            description=f"You've been credited 2 months of global No Prefix for boosting our Partnered Servers. You can now use my commands without prefix. If you wish to remove it, please reach out [Support Server](PASTE_YOUR_OWN_SUPPORT_SERVER_INVITE_HERE).",
                            color=0xFF0000
                        )
        try:
    ...
    async def remove_np(self, user):
        ...
        embed= discord.Embed(title="<a:Warningggg:1536000415601004564> Global No Prefix Expired",
                        description=f"Hey {user.mention}, your global no prefix has expired!\n\n__**Reason:**__ Unboosting our partnered Server.\nIf you think this is a mistake then please reach out [Support Server](PASTE_YOUR_OWN_SUPPORT_SERVER_INVITE_HERE).",
                        color=0xFF0000)
    ...
    async def np_reset(self, ctx):
        ...
        async def yes_callback(interaction):
            ...
            success_embed = discord.Embed(
                title="<a:Verified:1535986177264324628> No-Prefix Reset Complete",
