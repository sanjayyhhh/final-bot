    ...
    async def _create(self, ctx, name, *, message):
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            async with db.execute("SELECT COUNT(*) FROM autoresponses WHERE guild_id = ?", (ctx.guild.id,)) as cursor:
                ...
                if count >= 20:
                    return await ctx.reply(embed=discord.Embed(title="<a:Cross:1536000244716802072> Error!",
            ...
            await db.commit()
            await ctx.reply(embed=discord.Embed(title="<a:Verified:1535986177264324628> Success",
