...
class Booster (Cog ):
    ...
    async def _boost_thumbnail (self ,ctx ,thumbnail_url :str ):
        ...
        if not self .url_pattern .match (thumbnail_url ):
            ...
            description ="<a:Cross:1536000244716802072> Please provide a valid URL.",
            color =self .color 
            ))
        ...
        embed =discord .Embed (
        color =self .color ,
        description ="<a:Verified:1535986177264324628> Successfully updated the boost thumbnail URL.",
    ...
    async def _boost_image (self ,ctx ,*,image_url :str ):
        ...
        embed =discord .Embed (
        color =self .color ,
        description ="<a:Verified:1535986177264324628> Successfully updated the boost image URL.",
        timestamp =ctx .message .created_at 
        )
        embed .set_author (name =ctx .author .name ,icon_url =ctx .author .display_avatar .url )
        await ctx .send (embed =embed )
    ...
    async def _boost_autodel (self ,ctx ,seconds :int ):
        ...
        if seconds <0 :
            await ctx .send (embed =discord .Embed (
            description ="<a:Cross:1536000244716802072> Auto-delete timer must be 0 or greater.",
            color =self .color 
            ))
        ...
        description =f"<a:Verified:1535986177264324628> Successfully set auto-delete timer to {seconds} seconds."
        if seconds ==0 :
            description ="<a:Verified:1535986177264324628> Auto-delete has been disabled."
    ...
    async def _boost_message (self ,ctx ):
        ...
        embed =discord .Embed (
        color =self .color ,
        title ="<a:Verified:1535986177264324628> Boost Message Setup",
        ...
        try :
            ...
            await ctx .send (embed =discord .Embed (
            description ="<a:timer:1536982553636905031> Timeout! Please try again.",
            color =self .color 
            ))
        ...
        embed =discord .Embed (
        color =self .color ,
        description ="<a:Verified:1535986177264324628> Successfully updated the boost message.",
        timestamp =ctx .message .created_at 
        )
        embed .set_author (name =ctx .author .name ,icon_url =ctx .author .display_avatar .url )
        await ctx .send (embed =embed )
    ...
    async def _boost_embed (self ,ctx ):
        ...
        embed =discord .Embed (
        color =self .color ,
        description =f"<a:Verified:1535986177264324628> Embed formatting has been **{status}**.",
        timestamp =ctx .message .created_at 
        )
        embed .set_author (name =ctx .author .name ,icon_url =ctx .author .display_avatar .url )
        await ctx .send (embed =embed )
    ...
    async def _boost_ping (self ,ctx ):
        ...
        embed =discord .Embed (
        color =self .color ,
        description =f"<a:Verified:1535986177264324628> Booster pinging has been **{status}**.",
        timestamp =ctx .message .created_at 
        )
        embed .set_author (name =ctx .author .name ,icon_url =ctx .author .display_avatar .url )
        await ctx .send (embed =embed )
    ...
    async def _boost_channel_add (self ,ctx ,channel :discord .TextChannel ):
        ...
        if len (channels )>=3 :
            embed =discord .Embed (
            color =self .color ,
            description ="<a:Cross:1536000244716802072> Maximum boost channel limit reached (3 channels).",
            timestamp =ctx .message .created_at 
            )
            embed .set_author (name =ctx .author .name ,icon_url =ctx .author .display_avatar .url )
            await ctx .send (embed =embed )
        ...
        if str (channel .id )in channels :
            embed =discord .Embed (
            color =self .color ,
            description ="<a:Cross:1536000244716802072> This channel is already in the boost channels list.",
            timestamp =ctx .message .created_at 
            )
            embed .set_author (name =ctx .author .name ,icon_url =ctx .author .display_avatar .url )
            await ctx .send (embed =embed )
        ...
        embed =discord .Embed (
        color =self .color ,
        description =f"<a:Verified:1535986177264324628> Successfully added {channel.mention} to boost channels list.",
        timestamp =ctx .message .created_at 
        )
        embed .set_author (name =ctx .author .name ,icon_url =ctx .author .display_avatar .url )
        await ctx .send (embed =embed )
    ...
    async def _boost_channel_remove (self ,ctx ,channel :discord .TextChannel ):
        ...
        if not channels :
            embed =discord .Embed (
            color =self .color ,
            description ="<a:Cross:1536000244716802072> No boost channels are currently set up.",
            timestamp =ctx .message .created_at 
            )
            embed .set_author (name =ctx .author .name ,icon_url =ctx .author .display_avatar .url )
            await ctx .send (embed =embed )
        ...
        if str (channel .id )not in channels :
            embed =discord .Embed (
            color =self .color ,
            description ="<a:Cross:1536000244716802072> This channel is not in the boost channels list.",
            timestamp =ctx .message .created_at 
            )
            embed .set_author (name =ctx .author .name ,icon_url =ctx .author .display_avatar .url )
            await ctx .send (embed =embed )
        ...
        embed =discord .Embed (
        color =self .color ,
        description =f"<a:Verified:1535986177264324628> Successfully removed {channel.mention} from boost channels list.",
        timestamp =ctx .message .created_at 
        )
        embed .set_author (name =ctx .author .name ,icon_url =ctx .author .display_avatar .url )
        await ctx .send (embed =embed )
    ...
    async def _boost_test (self ,ctx ):
        ...
        if not channels :
            embed =discord .Embed (
            color =self .color ,
            description ="<a:Cross:1536000244716802072> Please set up a boost channel first using `boost channel add #channel`.",
            timestamp =ctx .message .created_at 
            )
            embed .set_author (name =ctx .author .name ,icon_url =ctx .author .display_avatar .url )
            await ctx .send (embed =embed )
        ...
        if not channel :
            await ctx .send (embed =discord .Embed (
            description ="<a:Cross:1536000244716802072> The configured boost channel no longer exists.",
            color =self .color 
            ))
        ...
        try :
            ...
            await ctx .send (embed =discord .Embed (
            description ="<a:Cross:1536000244716802072> I don't have permission to send messages in the boost channel.",
            color =self .color 
            ))

        except Exception as e :
            await ctx .send (embed =discord .Embed (
            description =f"<a:Cross:1536000244716802072> An error occurred: `{str(e)}`",
            color =self .color 
            ))
    ...
    async def _boost_config (self ,ctx ):
        ...
        embed =discord .Embed (
        color =self .color ,
        title =f"<a:Nitro_Boost:1535986050189623380> Boost Configuration for {ctx.guild.name}"
        )
        ...
        embed .add_field (
        name ="Embed",
        value ="<a:Verified:1535986177264324628> Enabled"if data ["boost"]["embed"]else "<a:Cross:1536000244716802072> Disabled",
        inline =True 
        )

        embed .add_field (
        name ="Ping",
        value ="<a:Verified:1535986177264324628> Enabled"if data ["boost"]["ping"]else "<a:Cross:1536000244716802072> Disabled",
        inline =True 
        )
    ...
    async def _boost_reset (self ,ctx ):
        ...
        if not data ["boost"]["channel"]:
            embed =discord .Embed (
            description ="<a:Cross:1536000244716802072> No boost configuration found to reset.",
            color =self .color 
            )
            await ctx .send (embed =embed )
            return 
        ...
        embed =discord .Embed (
        description ="<a:Verified:1535986177264324628> Successfully reset all boost configuration.",
        color =self .color 
        )
        await ctx .send (embed =embed )
    ...
    async def _boostrole_add (self ,ctx ,role :discord .Role ):
        ...
        if len (roles )>=10 :
            embed =discord .Embed (
            description ="<a:Cross:1536000244716802072> Maximum boost role limit reached (10 roles).",
            color =self .color 
            )
            await ctx .send (embed =embed )
            return 
        ...
        if str (role .id )in roles :
            embed =discord .Embed (
            description =f"<a:Cross:1536000244716802072> {role.mention} is already a boost role.",
            color =self .color 
            )
            await ctx .send (embed =embed )
            return 
        ...
        embed =discord .Embed (
        description =f"<a:Verified:1535986177264324628> {role.mention} has been added as a boost role.",
        color =self .color 
        )
        await ctx .send (embed =embed )
    ...
    async def _boostrole_remove (self ,ctx ,role :discord .Role ):
        ...
        if not roles :
            embed =discord .Embed (
            description ="<a:Cross:1536000244716802072> No boost roles are currently configured.",
            color =self .color 
            )
            await ctx .send (embed =embed )
            return 
        ...
        if str (role .id )not in roles :
            embed =discord .Embed (
            description =f"<a:Cross:1536000244716802072> {role.mention} is not a boost role.",
            color =self .color 
            )
            await ctx .send (embed =embed )
            return 
        ...
        embed =discord .Embed (
        description =f"<a:Verified:1535986177264324628> {role.mention} has been removed from boost roles.",
        color =self .color 
        )
        await ctx .send (embed =embed )
    ...
    async def _boostrole_reset (self ,ctx ):
        ...
        embed =discord .Embed (
        description ="<a:Verified:1535986177264324628> Successfully cleared all boost roles.",
        color =self .color 
        )
        await ctx .send (embed =embed )
