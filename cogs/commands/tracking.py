...
EMOJI_INVITE = "<a:ArrowRed:1502278700178210826>"
    ...
    async def invites(self, ctx, member: discord.Member = None):
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                f"**Left:** `{left}`\n"
                f"**Rejoins:** `{rejoin}`\n\n"
                f"{EMOJI_INVITE} **Join our [Support Server](PASTE_YOUR_OWN_SUPPORT_SERVER_INVITE_HERE)**"
            )
        )
        embed.set_thumbnail(url=member.avatar.url if member.avatar else member.default_avatar.url)
        embed.set_footer(text="NEON~V2 | Official", icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
