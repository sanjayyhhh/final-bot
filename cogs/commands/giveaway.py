...
class Giveaway(commands.Cog):
    ...
    async def gstart(self, ctx,
        ...
        embed = discord.Embed(title=f"🎊 {prize}",
                              description=f"<a:ArrowRed:1502278700178210826> Winner(s): **{winners}**\n<a:ArrowRed:1502278700178210826> Hosted by {ctx.author.mention}\n<a:ArrowRed:1502278700178210826> Ends <t:{round(ends)}:R> (<t:{round(ends)}:f>)\n\n<a:ArrowRed:1502278700178210826> React with 🎊 to participate!", color=0xFF0000)
    ...
    async def gend(self, ctx, message_id = None):
        ...
        if message_id is not None:
            ...
            if len(users) < 1:
                await ctx.send(f"<a:Verified:1535986177264324628> Successfully Ended the giveaway in <#{int(re[6])}>")
