...
class Whitelist(commands.Cog):
    ...
    async def whitelist(self, ctx, member: discord.Member = None):
        if ctx.guild.member_count < 2:
            embed = discord.Embed(
                color=0xFF0000,
                description="<a:Cross:1536000244716802072> | Your Server Doesn't Meet My 30 Member Criteria"
            )
        ...
        if not is_owner and not check:
            embed = discord.Embed(title="<a:Cross:1536000244716802072> Access Denied",
                color=0xFF0000,
        ...
        if not antinuke or not antinuke[0]:
            embed = discord.Embed(
                color=0xFF0000,
                description=(
                    f"**{ctx.guild.name} Security Settings <:staffmanager71:1538162986374533120> \n"
                    "Ohh No! looks like your server doesn't enabled Antinuke\n\n"
                    "Current Status : <a:Cross:1536000244716802072>\n\n"
        ...
        if not member:
            ...
            embed.add_field(name="__**Usage**__", value=f"<a:ArrowRed:1502278700178210826> `{prefix}whitelist @user/id`\n<a:ArrowRed:1502278700178210826> `{prefix}wl @user`")
            embed.set_thumbnail(url=ctx.bot.user.avatar.url)
            return await ctx.send(embed=embed)
        ...
        embed.set_footer(text=f"NEON~V2 | Official")
        ...
        try:
            ...
            if interaction.data["custom_id"] == "catWl":
                ...
                embed_description = "\n".join(f"<a:Cross:1536000244716802072><a:Verified:1535986177264324628>> : **{name}**" for key, name in fields.items())
        ...
        if not antinuke or not antinuke[0]:
            embed = discord.Embed(
                ...
                description=(
                    f"**{ctx.guild.name} Security Settings <:staffmanager71:1538162986374533120> \n"
                    "Ohh NO! looks like your server doesn't enabled security\n\n"
                    "Current Status : <a:Cross:1536000244716802072>I\n\n"
                    f"To enable use `{pre}antinuke enable` **"
                )
            )
            return await ctx.send(embed=embed)
