...
class General(commands.Cog):
  ...
  async def poll(self, ctx: commands.Context, *, message):
    ...
    await msg.add_reaction("<a:Verified:1535986177264324628>")
    await msg.add_reaction("<a:Cross:1536000244716802072>")
  ...
  @commands.command(name="users", help="checks total users of NEON~V2.")
  @blacklist_check()
  @ignore_check()
  @commands.cooldown(1, 3, commands.BucketType.user)
  ...
  async def wizz(self, ctx: commands.Context):
    ...
    embed = discord.Embed(
      ...
      description=f"**<a:Warningggg:1536000415601004564>Successfully Wizzed {ctx.guild.name}**",
      color=self.color,
  ...
  async def invite(self, ctx: commands.Context):
      embed = discord.Embed(
          title="🛜 NEON~V2 Integration Hub!",
          description="```Empower your server with blazing-fast features and 24/7 support!```",
          color=0xFF0000
      )
      embed.set_thumbnail(url=self.bot.user.avatar.url)
      embed.add_field(
          name="<:link:1536000681956343928> Quick Actions",
          value=(
              ">>> **[Invite NEON~V2](Sorry! , This is a Private Bot)**\n"
              "**[Support Server](https://discord.gg/jj87n6fDQX)**"
          ),
          inline=False
      )
      embed.set_footer(
          text=f"NEON~V2 | Official",
