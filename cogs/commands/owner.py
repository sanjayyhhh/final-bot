...
# NOTE: This file used to redefine OWNER_IDS locally, hardcoded to the
# original developer's Discord ID. That silently overrode whatever you set
# in utils/config.py for every check in this file (is_owner_or_staff,
# badges, guildinfo, slist). It's been deleted — this file now uses the
# single OWNER_IDS list imported from utils/config.py above, like it should.
...
DISCORD_BADGE_EMOJIS = {
    "nitro": "<a:Nitro_Boost:1535986050189623380>",
    "boost": "<a:Nitro_Boost:1535986050189623380>",
    "staff": "<a:Staff:1538167256096178216>",
    "partner": "<:Ownership:1538163725473681439>",
    "hypesquad": "<:Event:1538161118969860169>",
    ...
    "early_supporter": "<a:CustomerSupport:1538160762084655164>",
    "bug_hunter_level_2": "<:BUG_HUNTER_LEVEL_2:1448949685841825954>",
    "early_verified_bot_developer": "<a:EarlyVerifiedBotDeveloper:1448949731777839184>",
    "discord_certified_moderator": "<:DiscordsTrustSafetyTeam:1536983743955476530>",
...
class Owner(commands.Cog):
    ...
    async def staff_add(self, ctx, user: discord.User):
        if user.id in self.staff:
            sonu = discord.Embed(title="<a:Warningggg:1536000415601004564>  Access Denied", description=f"{user} is already in the staff list.", color=0xFF0000)
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                await db.commit()
            sonu2 = discord.Embed(title="<a:Verified:1535986177264324628> Success", description=f"Added {user} to the staff list.", color=0xFF0000)
    ...
    async def staff_remove(self, ctx, user: discord.User):
        if user.id not in self.staff:
            sonu = discord.Embed(title="<a:Warningggg:1536000415601004564> Access Denied", description=f"{user} is not in the staff list.", color=0xFF0000)
            await ctx.reply(embed=sonu, mention_author=False)
        else:
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                await db.commit()
                sonu2 = discord.Embed(title="<a:Verified:1535986177264324628> Success", description=f"Removed {user} from the staff list.", color=0xFF0000)
            await ctx.reply(embed=sonu2, mention_author=False)
    ...
    async def staff_list(self, ctx):
        if not self.staff:
            ...
            staff_display = "\n".join(member_list)
            sonu = discord.Embed(title="<a:Verified:1535986177264324628> NEON~V2 Staffs", description=f"\n{staff_display}", color=0xFF0000)
    ...
    async def _slist(self, ctx):
        ...
        paginator = Paginator(source=DescriptionEmbedPaginator(
            entries=entries,
            description="",
            title=f"Guild List of NEON~V2 [{len(self.client.guilds)}]",
            color=0xFF0000,
            per_page=10),
            ctx=ctx)
    ...
    async def _restart(self, ctx: Context):
        await ctx.reply("<a:Verified:1535986177264324628> | **Successfully Restarting NEON~V2, It Takes 10 seconds**")
        restart_program()
    ...
    async def own_list(self, ctx):
        ...
        paginator = Paginator(source=DescriptionEmbedPaginator(
            entries=entries,
            title=f"NEON~V2 Owners [{len(nplist)}]",
            description="",
            per_page=10,
            color=0xFF0000),
                              ctx=ctx)
    ...
    async def dm(self, ctx, user: discord.User, *, message: str):
        ...
        try:
            ...
            await ctx.send(f"<a:Verified:1535986177264324628> | Successfully Sent a DM to **{user}**")
        except discord.Forbidden:
    ...
    async def change_nickname(self, ctx, *, name: str = None):
        ...
        try:
            ...
            if name:
                await ctx.send(f"<a:Verified:1535986177264324628> | Successfully changed nickname to **{name}**")
            else:
                await ctx.send("<a:Verified:1535986177264324628> | Successfully removed nickname")
        ...
        if member:
            try:
                ...
                embed = discord.Embed(
                    ...
                    description=f"<a:Verified:1535986177264324628> | **{member.name}** has been successfully banned from {ctx.guild.name} by the Bot Owner.",
                    color=0xFF0000)
                ...
                embed = discord.Embed(
                    title="Error!",
                    description=f"<a:Warningggg:1536000415601004564>  I do not have permission to ban **{member.name}** in this guild.",
                    color=0xFF0000
                )
                ...
                embed = discord.Embed(
                    title="Error!",
                    description=f"<a:Warningggg:1536000415601004564>  An error occurred while banning **{member.name}**.",
                    color=0xFF0000
                )
                await ctx.reply(embed=embed, mention_author=False, delete_after=5)
                await ctx.message.delete()
        ...
        if user:
            try:
                ...
                embed = discord.Embed(
                    ...
                    description=f"<a:Verified:1535986177264324628> | **{user.name}** has been successfully unbanned from {ctx.guild.name} by the Bot Owner.",
                    color=0xFF0000
                )
                ...
                embed = discord.Embed(
                    title="Error!",
                    description=f"<a:Warningggg:1536000415601004564>  I do not have permission to unban **{user.name}** in this guild.",
                    color=0xFF0000
                )
                await ctx.reply(embed=embed, mention_author=False)
            except discord.HTTPException:
                embed = discord.Embed(
                    title="Error!",
                    description=f"<a:Warningggg:1536000415601004564>  An error occurred while unbanning **{user.name}**.",
                    color=0xFF0000
                )
                await ctx.reply(embed=embed, mention_author=False)
        else:
...
class Badges(commands.Cog):
    ...
    async def profile(self, ctx: commands.Context, member: discord.Member = None):
        ...
        loading_embed = discord.Embed(
            title="<a:loading:1536975745681727538> Loading Profile...",
            color=0xFF0000
        )
