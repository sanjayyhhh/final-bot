...
class joindm(commands.Cog):
    ...
    async def joindm(self, ctx):
        ...
        if guild_id in self.joindm_messages:
            await ctx.send(f"**<a:Verified:1535986177264324628> | The current join DM message is: `{self.joindm_messages[guild_id]}`")
        else:
            await ctx.send("**<a:Cross:1536000244716802072> | No custom join DM message has been set for this server. **")
    ...
    async def message(self, ctx, *, message=None):
        ...
        if message is None:
            await ctx.send("** <a:Cross:1536000244716802072> | Please provide a custom join DM message.**")
        else:
            self.joindm_messages[str(ctx.guild.id)] = message
            self.save_joindm_messages()
            await ctx.send("** <a:Verified:1535986177264324628> | Custom join DM message set successfully.**")
    ...
    async def enable(self, ctx):
        ...
        await ctx.send("**<a:Verified:1535986177264324628> | Join DM module enabled. Custom DM will be sent to new members.**")
    ...
    async def disable(self, ctx):
        ...
        await ctx.send("**<a:Verified:1535986177264324628> | Join DM module disabled. Custom DM will not be sent to new members.**")
    ...
    async def test(self, ctx):
        ...
        if guild_id in self.joindm_messages:
            ...
            await ctx.send("🕶️ Test Sent To Your Dm")
            await ctx.message.add_reaction("<:magicblob:1538165520526741514>")
