...
""")
    ...
    async def enable(self, ctx):
        if ctx.author.id != ctx.guild.owner_id:
            embed = discord.Embed(title="<:zcross:1448951756372443296> Error", description="Only the server owner can enable emergency mode.", color=0x000000)
        ...
        if roles_added:
            ...
            embed = discord.Embed(title="<:ztick:1448951767990796298> Success", description=f"The following roles with dangerous permissions have been added to the **emergency list**:\n{description}", color=0x000000)
    ...
    async def authorise_add(self, ctx, member: discord.Member):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            if count >= 5:
                embed = discord.Embed(title="<:zwarning:1448949627712966717> Access Denied", description="Only up to 5 authorised users can be added.", color=0x000000)
                return await ctx.reply(embed=embed)
    ...
    async def authorise_remove(self, ctx, member: discord.Member):
        if not await self.is_guild_owner(ctx):
            embed = discord.Embed(title="<:zwarning:1448949627712966717> Access Denied", description="Only the server owner can remove authorised users for emergency situation.", color=0x000000)
            return await ctx.reply(embed=embed)
    ...
    async def list_authorized(self, ctx):
        if not await self.is_guild_owner(ctx):
            embed = discord.Embed(title="<:zwarning:1448949627712966717> Access Denied", description="Only the server owner can view the list of authorised users for emergency situation.", color=0x000000)
            return await ctx.reply(embed=embed)
    ...
    async def role_add(self, ctx, role: discord.Role):
        if not await self.is_guild_owner(ctx):
            embed = discord.Embed(title="<:zwarning:1448949627712966717> Access Denied", description="Only the server owner can add role for emergency situation.", color=0x000000)
            return await ctx.reply(embed=embed)
        ...
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute("SELECT COUNT(*) FROM emergency_roles WHERE guild_id = ?", (ctx.guild.id,)) as cursor:
                count = (await cursor.fetchone())[0]
            if count >= 25:
                embed = discord.Embed(title="<:zwarning:1448949627712966717> Error", description="Only up to 25 roles can be added.", color=0x000000)
                return await ctx.reply(embed=embed)
    ...
    async def role_remove(self, ctx, role: discord.Role):
        if not await self.is_guild_owner(ctx):
            embed = discord.Embed(title="<:zwarning:1448949627712966717> Access Denied", description="Only the server owner can remove roles from emergency list.", color=0x000000)
            return await ctx.reply(embed=embed)
    ...
    async def list_roles(self, ctx):
        if not await self.is_guild_owner_or_authorised(ctx):
            embed = discord.Embed(title="<:zwarning:1448949627712966717> Access Denied", description="You are not authorised to view list of roles for emergency situation.", color=0x000000)
            return await ctx.reply(embed=embed)
    ...
    async def emergencysituation(self, ctx):
        ...
        if not await self.is_guild_owner_or_authorised(ctx):
            return await ctx.reply(embed=discord.Embed(
                title="<:zwarning:1448949627712966717> Access Denied", 
