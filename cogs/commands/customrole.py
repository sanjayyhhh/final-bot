...
class Customrole(commands.Cog):
    ...
    async def add_role(self, *, role_id: int, member: discord.Member):
        if member.guild.me.guild_permissions.manage_roles:
            ...
            await member.add_roles(role, reason="NEON~V2 Customrole | Role Added")
        else:
    ...
    async def remove_role(self, *, role_id: int, member: discord.Member):
        if member.guild.me.guild_permissions.manage_roles:
            role = discord.Object(id=role_id)
            await member.remove_roles(role, reason="NEON~V2 Customrole | Role Removed")
        else:
            raise discord.Forbidden("Bot does not have permission to manage roles.")
                        ...
                        if context.author == context.guild.owner or reqrole in context.author.roles:
                            if role:
                                if role not in member.roles:
                                    ...
                                    embed = discord.Embed(title="<a:Verified:1535986177264324628> Success",
                                        description=f"**Given** <@&{role.id}> To {member.mention}",
                                        color=0xFF0000)
                                else:
                                    await self.remove_role2(role=role_id, member=member)
                                    embed = discord.Embed(title="<a:tick:1538163587833663590> Success",
                                        description=f"**Removed** <@&{role.id}> From {member.mention}",
                                        color=0xFF0000)
                                await context.reply(embed=embed)
                            else:
                                embed = discord.Embed(title="<a:Cross:1536000244716802072> Error",
    ...
    async def reset(self, context: Context) -> None:
        if context.author == context.guild.owner or context.author.top_role.position > context.guild.me.top_role.position:
            ...
            description=f"Deleted All Custom Role commands <a:tick:1538163587833663590>\n\n**Removed Roles:**\n" + "\n".join(removed_roles) if removed_roles else "No roles were previously set.",
            color=0x000000
        )
    ...
    async def on_message(self, message: discord.Message):
        ...
        if result:
            ...
            if reqrole is None:
                await message.channel.send("<a:Warningggg:1536000415601004564> The required role is not set up in this server. Please set it up using `setup reqrole`.")
                return

            
            if reqrole not in message.author.roles:
                await message.channel.send(embed=discord.Embed(description=f"<a:Warningggg:1536000415601004564> You need the {reqrole.mention} role to use this command.", color=0x000000))
                return
            ...
            try:
                if role in member.roles:
                    ...
                    await message.channel.send(embed=discord.Embed(
                        title="<a:tick:1538163587833663590> Success",
