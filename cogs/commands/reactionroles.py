...
class ReactionRoles(commands.Cog):
    ...
    async def createrr(self, ctx: Context, channel: discord.TextChannel, message_id: int, emoji: str, role: discord.Role):
        try:
            ...
            await ctx.send(f"<a:Verified:1535986177264324628> Reaction role added: React with {emoji} to get {role.name}", ephemeral=True if ctx.interaction else False)
        except discord.NotFound:
            await ctx.send("<a:Cross:1536000244716802072> Message not found.", ephemeral=True if ctx.interaction else False)
        except discord.HTTPException as e:
            await ctx.send(f"<a:Cross:1536000244716802072>  Error: {str(e)}", ephemeral=True if ctx.interaction else False)
    ...
    async def dmrr(self, ctx: Context, mode: str):
        if mode.lower() not in ["enable", "disable"]:
            await ctx.send("<a:Cross:1536000244716802072> Use `enable` or `disable`.", ephemeral=True if ctx.interaction else False)
            return

        value = 1 if mode.lower() == "enable" else 0
        self.set_dm_setting(ctx.guild.id, value)
        await ctx.send(f"<a:Verified:1535986177264324628> DM messages for reaction roles {'enabled' if value else 'disabled'}.", ephemeral=True if ctx.interaction else False)
        ...
        if role_id:
            ...
            if role and member:
                ...
                if self.get_dm_setting(payload.guild_id):
                    try:
                        await member.send(f"<a:Verified:1535986177264324628> You received the **{role.name}** role from {guild.name}.")
