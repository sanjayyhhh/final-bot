...
class Antinuke(commands.Cog):
  ...
  async def antinuke(self, ctx, option: str = None):
    ...
    if not is_owner and not check:
      embed = discord.Embed(title="<a:Warningggg:1536000415601004564> Access Denied",
                color=0xFF0000,
    ...
    if option is None:
      embed = discord.Embed(
        title='<:DiscordsTrustSafetyTeam:1536983743955476530> NEON~V2 Security',
        description="**Antinuke Defense Mode** — Protect your server from harmful admin actions with smart automated security protocols.\n\n"
            ...
            "**Configuration Panel**\n"
            "<a:Verified:1535986177264324628> Enable Protection: `antinuke enable`\n"
            "<a:Warningggg:1536000415601004564> Disable Protection: `antinuke disable`",
        color=0xFF0000
      )      
      embed.set_thumbnail(url=self.bot.user.avatar.url)
      embed.set_footer(text="NEON~V2 — Your 24/7 Security Partner.", icon_url=self.bot.user.avatar.url)
      await ctx.send(embed=embed)

    elif option.lower() == 'enable':
      if is_activated:
        embed = discord.Embed(
          description=f'**Security Settings For {ctx.guild.name}**\nYour server __**already has Antinuke enabled.**__\n\nCurrent Status: <a:Verified:1535986177264324628> Enabled\nTo Disable use `antinuke disable`',
          color=0xFF0000
        )
        embed.set_thumbnail(url=self.bot.user.avatar.url)
        await ctx.send(embed=embed)
        ...
        setup_embed = discord.Embed(
          title="Antinuke Setup <a:six_sevennn:1536978639453822996>",
          description="<a:Verified:1535986177264324628> | Initializing Quick Setup!",
          color=0xFF0000
        )
        ...
        if not ctx.guild.me.guild_permissions.administrator:
          setup_embed.description += "\n<a:Warningggg:1536000415601004564> | **Ops! It seems I Don't Have Administrator Perm To enable antinuke**."
          await setup_message.edit(embed=setup_embed)
          return

        await asyncio.sleep(1)
        setup_embed.description += "\n<a:Verified:1535986177264324628> Checking NEON~V2's role position for optimal configuration..."
        await setup_message.edit(embed=setup_embed)

        await asyncio.sleep(1)
        setup_embed.description += "\n<a:Verified:1535986177264324628> | Crafting and configuring the NEON~V2 Supreme role..."
        await setup_message.edit(embed=setup_embed)
        
        try:
          role = await ctx.guild.create_role(
            name="NEON~V2 Supreme™",
            color=0xFF0000,
          ...
          setup_embed.description += "\n<a:Warningggg:1536000415601004564> | **Uh oh! I don't Have perms to enable antinuke**."
          await setup_message.edit(embed=setup_embed)
          return
        except discord.HTTPException as e:
          setup_embed.description += f"\n<a:Warningggg:1536000415601004564> | **Uh: HTTPException: {e}\nCheck Guild Audit Logs**."
          await setup_message.edit(embed=setup_embed)
          return

        await asyncio.sleep(1)
        setup_embed.description += "\n<a:Verified:1535986177264324628>| Ensuring precise placement of the NEON~V2 Supreme™ role..."
        await setup_message.edit(embed=setup_embed)
        try:
          await ctx.guild.edit_role_positions(positions={role: 1})
        except discord.Forbidden:
          setup_embed.description += "\n<a:Warningggg:1536000415601004564> | Ops! I don't have sufficient perms to move role."
          await setup_message.edit(embed=setup_embed)
          return
        except discord.HTTPException as e:
          setup_embed.description += f"\n<a:Warningggg:1536000415601004564> | Setup failed: HTTPException: {e}."
          await setup_message.edit(embed=setup_embed)
          return

        await asyncio.sleep(1)
        setup_embed.description += "\n<a:Verified:1535986177264324628> | Safeguarding your changes..."
        await setup_message.edit(embed=setup_embed)

        await asyncio.sleep(1)
        setup_embed.description += "\n<a:Verified:1535986177264324628> | Activating the Antinuke Modules for enhanced security...!!"
        await setup_message.edit(embed=setup_embed)
        ...
        embed = discord.Embed(
          description=f"**Security Settings For {ctx.guild.name} **\n\nTip: For optimal functionality of the AntiNuke Module, please ensure that my role has **Administration** permissions and is positioned at the **Top** of the roles list\n\n<a:lock_key:1535986624788439041> __**Modules Enabled**__\n>>> <a:Verified:1535986177264324628> **Anti Ban**\n<a:Verified:1535986177264324628> **Anti Kick**\n<a:Verified:1535986177264324628> **Anti Bot**\n<a:Verified:1535986177264324628> **Anti Channel Create**\n<a:Verified:1535986177264324628> **Anti Channel Delete**\n<a:Verified:1535986177264324628> **Anti Channel Update**\n<a:Verified:1535986177264324628> **Anti Everyone/Here**\n<a:Verified:1535986177264324628> **Anti Role Create**\n<a:Verified:1535986177264324628> **Anti Role Delete**\n<a:Verified:1535986177264324628> **Anti Role Update**\n<a:Verified:1535986177264324628> **Anti Member Update**\n<a:Verified:1535986177264324628> **Anti Guild Update**\n<a:Verified:1535986177264324628> **Anti Integration**\n<a:Verified:1535986177264324628> **Anti Webhook Create**\n<a:Verified:1535986177264324628> **Anti Webhook Delete**\n<a:Verified:1535986177264324628> **Anti Webhook Update**",
          color=0xFF0000
        )

        embed.add_field(name='', value=">>> <a:Verified:1535986177264324628> **Anti Prune**\n<a:Verified:1535986177264324628> **Auto Recovery**")

        embed.set_author(name="NEON~V2 Antinuke", icon_url=self.bot.user.avatar.url)

        embed.set_footer(text="Successfully Enabled Antinuke for this server | NEON~V2 | Official", icon_url=self.bot.user.avatar.url)
        embed.set_thumbnail(url=self.bot.user.avatar.url)
      ...
      if not is_activated:
        embed = discord.Embed(
          description=f'**Security Settings For {ctx.guild.name}**\nUhh, looks like your server hasn\'t enabled Antinuke.\n\nCurrent Status: <a:Warningggg:1536000415601004564> Disabled\n\nTo Enable use `antinuke enable`',
          color=0xFF0000
        )
        embed.set_thumbnail(url=self.bot.user.avatar.url)
      else:
