    ...
    async def on_submit (self ,interaction :discord .Interaction ):
        try :
            ...
            try :
                ...
                await interaction .followup .send ("<a:Cross:1536000244716802072> Invalid XP per message value! Must be between 1 and 999.",ephemeral =True )
                return 
            ...
            if not validate_hex_color (color_value ):
                await interaction .followup .send ("<a:Cross:1536000244716802072> Invalid hex color format! Use format like #FF0000",ephemeral =True )
                return 
            ...
            embed =discord .Embed (
            title ="<a:Verified:1535986177264324628> Leveling Configuration Updated",
            ...
            f"<a:Cross:1536000244716802072> An unexpected error occurred: {str(e)}",
            ephemeral =True 
            )
...
└─────────────────────────────────────────┘
    ...
    async def interactive_setup (self ,ctx ,current_settings ):
        ...
        try :
            ...
            value =f"🔧 Status: {'<a:Verified:1535986177264324628> Enabled' if current_settings.get('enabled', False) else '<a:Cross:1536000244716802072> Disabled'}\n"
            f"💎 XP per Message: {current_settings.get('xp_per_message', 20)}\n"
            f"🎨 Embed Color: {current_settings.get('embed_color', '#000000')}\n"
            f"🖼️ Thumbnail: {'<a:Verified:1535986177264324628> Yes' if current_settings.get('thumbnail_enabled', True) else '<a:Cross:1536000244716802072> No'}\n"
            ...
            try :
                ...
                if xp_msg .content .lower ()=='cancel':
                    await ctx .send ("<a:Cross:1536000244716802072> Setup cancelled.")
                    return 
                ...
                if xp_msg .content .lower ()=='skip':
                    ...
                    if xp_per_message <1 or xp_per_message >999 :
                        await ctx .send ("<a:Cross:1536000244716802072> XP must be between 1 and 999. Using current value.")
                        xp_per_message =current_settings .get ('xp_per_message',20 )
            ...
            try :
                ...
                if msg_response .content .lower ()=='skip':
                    ...
                    if len (level_message )>2000 :
                        await ctx .send ("<a:Cross:1536000244716802072> Message too long (max 2000 chars). Using current value.")
                        level_message =current_msg 
            ...
            try :
                ...
                if color_msg .content .lower ()=='skip':
                    ...
                    if validate_hex_color (color_input ):
                        ...
                        await ctx .send ("<a:Cross:1536000244716802072> Invalid hex color. Using current value.")
                        embed_color =current_color 
            except asyncio .TimeoutError :
                await ctx .send ("⏰ Timeout. Using current value.")
                embed_color =current_color 
            ...
            try :
                ...
                if image_msg .content .lower ()=='skip':
                    ...
                    if level_image and len (level_image )>500 :
                        await ctx .send ("<a:Cross:1536000244716802072> URL too long. Using current value.")
                        level_image =current_image 
            except asyncio .TimeoutError :
                await ctx .send ("⏰ Timeout. Using current value.")
                level_image =current_image 
            ...
            title ="<a:Verified:1535986177264324628> Leveling System Configured!",
    ...
    async def setxp (self ,ctx ,amount :int ):
        ...
        try :
            if amount <1 or amount >999 :
                await ctx .send ("<a:Cross:1536000244716802072> XP per message must be between 1 and 999!")
                return 
            ...
            embed =discord .Embed (
            title ="<a:Verified:1535986177264324628> XP Amount Updated",
    ...
    async def setmessage (self ,ctx ,*,message :str ):
        ...
        try :
            if len (message )>2000 :
                await ctx .send ("<a:Cross:1536000244716802072> Level-up message must be less than 2000 characters!")
                return 
            ...
            embed =discord .Embed (
            title ="<a:Verified:1535986177264324628> Level-Up Message Updated",
    ...
    async def setcolor (self ,ctx ,color :str ):
        ...
        try :
            ...
            if not validate_hex_color (color ):
                await ctx .send ("<a:Cross:1536000244716802072> Invalid hex color format! Use format like #FF0000")
                return 
            ...
            embed =discord .Embed (
            title ="<a:Verified:1535986177264324628> Embed Color Updated",
    ...
    async def thumbnail (self ,ctx ,setting :str ):
        ...
        try :
            ...
            if setting not in ['on','off','true','false','yes','no','enable','disable']:
                await ctx .send ("<a:Cross:1536000244716802072> Use: `on/off`, `true/false`, `yes/no`, or `enable/disable`")
                return 
            ...
            embed =discord .Embed (
            title ="<a:Verified:1535986177264324628> Thumbnail Setting Updated",
    ...
    async def cooldown (self ,ctx ,seconds :int ):
        ...
        try :
            if seconds <0 or seconds >3600 :
                await ctx .send ("<a:Cross:1536000244716802072> Cooldown must be between 0 and 3600 seconds (1 hour)!")
                return 
            ...
            embed =discord .Embed (
            title ="<a:Verified:1535986177264324628> Cooldown Updated",
    ...
    async def enable (self ,ctx ):
        try :
            async with aiosqlite .connect (self .db_path )as db :
                ...
                if result :
                    if result [0 ]==1 :
                        await ctx .send ("<a:Warningggg:1536000415601004564> Leveling system is already enabled!")
                        return 
            ...
            embed =discord .Embed (
            title ="<a:Verified:1535986177264324628> Leveling System Enabled",
    ...
    async def disable (self ,ctx ):
        try :
            async with aiosqlite .connect (self .db_path )as db :
                ...
                if not result :
                    await ctx .send ("<a:Warningggg:1536000415601004564> Leveling system is not configured for this server!")
                    return 

                if result [0 ]==0 :
                    await ctx .send ("<a:Warningggg:1536000415601004564> Leveling system is already disabled!")
                    return 
            ...
            embed =discord .Embed (
            title ="<a:Cross:1536000244716802072> Leveling System Disabled",
