...
class Guild(Cog):
    ...
    async def on_guild_add(self, guild):
        try:
            ...
            embed.add_field(
                name="**__Members__**",
                value=f"""<a:rockandrole:1535985141107662959> Members : {len(guild.members)}\n 🧑‍🤝‍🧑 Humans : {len(list(filter(lambda m: not m.bot, guild.members)))}\n <:bots:1536976337787686912> Bots : {len(list(filter(lambda m: m.bot, guild.members)))}
                """,
                inline=False
            )
            embed.add_field(
