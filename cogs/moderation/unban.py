...
#class ReasonModal(ui.Modal):
   # def __init__(self, user, author, view):
    ...
    #async def on_submit(self, interaction: discord.Interaction):
     ...
     #   try:
          #  await self.user.send(f"<a:Warningggg:1536000415601004564> You have been Banned from **{self.author.guild.name}** by **{self.author}**. Reason: {reason or 'No reason provided'}")
         #   dm_status = "Yes"
...
class Unban(commands.Cog):
    ...
    async def unban(self, ctx, user: discord.User, *, reason=None):
        ...
        try:
            await user.send(f"<a:Verified:1535986177264324628> You have been unbanned from **{ctx.guild.name}** by **{ctx.author}**. Reason: {reason or 'No reason provided'}")
            dm_status = "Yes"
        except discord.Forbidden:
            dm_status = "No"
        except discord.HTTPException:
        ...
        embed = discord.Embed(description=f"**<a:Verified:1535986177264324628> | Successfully Unbaned [{user}](https://discord.com/users/{user.id})\nReason {reasonn}**", color=0xFF0000)
