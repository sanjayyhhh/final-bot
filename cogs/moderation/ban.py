...
class Ban(commands.Cog):
    ...
    async def ban(self, ctx, user: discord.User, *, reason=None):
        ...
        # Try to DM the user
        try:
            await user.send(f"<a:Warningggg:1536000415601004564> You have been banned from **{ctx.guild.name}** by **{ctx.author}**. Reason: {reason or 'No reason provided'}")
            dm_status = "Yes"
        ...
        container.add_item(ui.TextDisplay(
            f"**<a:Verified:1535986177264324628> | [{user}](https://discord.com/users/{user.id}) Has Been Banned Successfully**"
