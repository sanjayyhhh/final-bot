...
class Lock(commands.Cog):
    ...
    async def lock_command(self, ctx: commands.Context, channel: discord.TextChannel = None):
        ...
        if not channel.permissions_for(ctx.guild.default_role).send_messages:
            embed = discord.Embed(
                description=f"**Channel: {channel.mention}\n<a:Verified:1535986177264324628> Status: Already Locked**",
                color=0xFF0000
            )
        ...
        embed = discord.Embed(
            title="NEON~V2 | Lockdown",
            description=f"<a:Verified:1535986177264324628> | Successfully Locked {channel.mention}",
            color=0xFF0000
        )
