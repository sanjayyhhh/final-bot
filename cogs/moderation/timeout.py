...
#
   # @ui.button(style=discord.ButtonStyle.gray, emoji="<:remove:1538160153449205851>")
...
class Mute(commands.Cog):
    ...
    async def mute(self, ctx, user: discord.Member, time: str = None, *, reason=None):
        ...
        try:
            await user.send(f"<a:Warningggg:1536000415601004564> You have been muted in **{ctx.guild.name}** by **{ctx.author}** for {duration_text}. Reason: {reason or 'None'}")
            dm_status = "Yes"
        except discord.Forbidden:
            dm_status = "No"
        except discord.HTTPException:
        ...
        embed = discord.Embed(description=f"**<a:Verified:1535986177264324628> | Successfully Muted [{user}](https://discord.com/users/{user.id}) For {duration_text}\nReason {reason}**",
                              color=self.color)
    ...
    async def mute_error(self, ctx, error):
        ...
        if isinstance(error, commands.BotMissingPermissions):
            embed = discord.Embed(title="<a:Cross:1536000244716802072>> Access Denied", description="I don't have permission to mute members.", color=self.color)
            await ctx.send(embed=embed)
        elif isinstance(error, discord.Forbidden):
            embed = discord.Embed(title="<a:Cross:1536000244716802072> Missing Permissions", description="I can't mute this user as they might have higher privileges (e.g., Admin).", color=self.color)
            await ctx.send(embed=embed)
            
        else:
            embed = discord.Embed(title="<a:Cross:1536000244716802072> Unexpected Error", description=str(error), color=self.color)
            await ctx.send(embed=embed)
